...
\begin{equation}
\max_{\theta, \phi, v, t_{drop}, \Delta t}
T_{occ} = \sum_{t} \mathbb{1}\!\left[\exists (j,k): \text{屏蔽}(t, j, k)\right] \cdot \Delta t
\label{eq:obj}
\end{equation}
约束条件集中表示为：
\begin{equation}
...
\label{eq:cons}
\end{equation}
其中"屏蔽$(t, j, k)$"定义为 $t$ 时刻视线段与球 $C_{j,k}(t)$ 且与圆柱 $O_{true}$ 同时有交（参见公式 \eqref{eq:cover} 的几何求交判定）。
