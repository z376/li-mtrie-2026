...
- **AI 读图**（PNG 预览 + student 用 Mavis/Claude/GPT 复看）抓**感知性**问题：图例压数据、
...
def audit_layout(fig, clip_tol_px: float = 2.0, overlap_tol_px: float = 1.0
    ...
    if glyph_msgs:
        ...
        issues.append((
            ...
            "中文图请先在 code-template.py 顶部用 matplotlib.rcParams['font.sans-serif'] 配置 CJK 字体（优先级 SimHei > Microsoft YaHei > Arial Unicode MS > DejaVu Sans）；"
...
def print_report(issues: list[tuple[str, str]]) -> str:
    ...
    if not issues:
        ...
        print("  >>> 仍需 AI 读图复核感知性问题（用 render_preview() 出 PNG 预览，Mavis/Claude/GPT 复看）。")
        return "PASS"
