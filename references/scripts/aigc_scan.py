...
# Windows GBK console 兼容 (aigc_scan.py 用 emoji ✅/⚠️, 必修)
# Python 3.7+
try:
...
# 词级替换应配合 references/受保护片段.md 的 5 类禁改边界使用。
COLLOQUIAL_TERMS = [
    # 网络用语
...
def load_text(filepath: str) -> str:
    """读取文本文件, 自动尝试 4 种编码 (utf-8 / utf-8-sig / gbk / gb18030).

    Raises:
        FileNotFoundError: 当 filepath 不存在时抛出 (而非 sys.exit, 这样作为库
            被其他脚本 import 时不会杀死调用进程)
    """
    path = Path(filepath)
    if not path.exists():
        raise FileNotFoundError(f"文件不存在 — {filepath}")

    # 自动尝试多种编码, 适配 Windows GBK 中文论文 / UTF-8 BOM Excel 导出
    ...
    raise UnicodeDecodeError(
        f"无法用 utf-8/utf-8-sig/gbk/gb18030 解码 {filepath}, 请手动转码")
...
def split_sentences(text: str) -> list[str]:
    """按句子拆分. 句号切分时跳过 e.g./i.e./etc./数字. 的情况, 避免误切缩写.

    Python 3.10+ re 引擎对 unicode \\b lookbehind 行为已稳定, 此正则工作 OK.
    """
    # \be/\bg/\bi lookbehind: 保护 e.g./i.e./ 缩写; \d lookbehind + lookahead: 保护数字. 数字
