"""li-mtrie-2026 赛前 1 天必做 + CI smoke-test 一体化脚本 (v1.5.7.6).
...
- v1.5.3 patch4: 加 8 项 CI 检查 (frontmatter / Python 10 个 / LLM 工具 / BZD 借鉴 /
  ...
  连 fail 的根因).
- v1.5.7.6 patch: 加 3 项 v1.5.7.6 审计 fix 覆盖 (profile_data warnings 遮蔽 / sd 键名错 /
  aigc_scan sys.exit), 凑齐 23 项.
- v1.5.8 patch: 加 4 项 v1.5.8 BZD 借鉴覆盖 (读题逐句翻译 / 建模方案框架 / 端到端工作流对照
  / 学校画像与备赛评估), 凑齐 27 项.
- v1.5.8.1 hotfix: 加 2 项 v1.5.8.1 BZD 配套 LLM prompt 覆盖 (06-读题翻译 / 07-建模方案),
  凑齐 29 项. 同日小修补, 不算 minor bump.
- v1.5.8.2 hotfix: writing-for-agents 审计 P0 3 项 (cache 精简 8 处 + description 触发词
  校准 1 处 + 4 references 完成判据 13 个). 不加新 check, 只做审计修.
- v1.5.8.3 hotfix: writing-for-agents 审计 P1 3 项 (negation 改 positive 4 处 + 4 references
  step 锚定 + 借鉴段顺序按触发顺序重排). 不加新 check, 只做审计修.
- v1.5.8.4 hotfix: writing-for-agents 审计 P2 3 项 (LLM prompt 标题中英一致 2 处 + 4 references
  §1 何时用 改完成判据 + README 协同段改表格). 不加新 check, 只做审计修.
- v1.5.9 minor: 加深借鉴 BZD bzd-paper-aigc-auditor v2.2 (LLM agent + HTML 报告).
  新增 1 LLM prompt (08-AIGC审计) + 1 HTML 报告模板 (自己重写简化版) + 扩展
  09-AIGC审计.md. 加 2 check (30/31) 验配套, 凑齐 31 项.

用法:
    python references/scripts/dryrun.py            # 跑 31 项 checkable
...
LLM_PROMPTS_DIR = SKILL_ROOT / "references" / "llm-prompts"
...
def check_no_warnings_shadow():
    """checkable 21: profile_data.py 不应再用 `warnings` 局部变量遮蔽 stdlib `warnings` 模块.

    背景: v1.5.7.6 审计发现 profile_data.py L353 `warnings: list[str] = []` 遮蔽
    `import warnings` 模块, 后续 `warnings.catch_warnings()` 调用静默失败.
    v1.5.7.6 修复: 列表重命名为 `warns`. 此 check 防回退.
    """
    target = SCRIPTS_DIR / "profile_data.py"
    if not target.exists():
        return ("yellow", f"{target.name} 不存在, 跳过", "检 skill 完整性")
    try:
        content = target.read_text(encoding="utf-8", errors="replace")
    except Exception as e:
        return ("red", f"读 {target.name} 失败: {e}", "查文件权限/编码")
    # 匹配列表用法的 `warnings` (排除 stdlib 模块用法 `import warnings` / `warnings.catch_warnings()` 等)
    bad_patterns = [
        (r"^\s+warnings\s*:\s*list\[str\]", "局部 list 变量声明 `warnings: list[str]`"),
        (r"^\s+warnings\s*=\s*\[\]", "局部 list 变量声明 `warnings = []`"),
        (r"^\s+warnings\.append\(", "列表 append `warnings.append(...)`"),
    ]
    hits = []
    for pat, desc in bad_patterns:
        for m in re.finditer(pat, content, re.MULTILINE):
            line_no = content[:m.start()].count("\n") + 1
            hits.append(f"L{line_no}: {desc}")
    if hits:
        return ("red",
                f"profile_data.py 出现 {len(hits)} 处 `warnings` 列表用法 (遮蔽 stdlib `warnings` 模块): {hits[:3]}",
                "把局部列表变量重命名为 `warns`, 保留 `import warnings` 模块和 `warnings.catch_warnings()` 不动")
    return ("green", "profile_data.py 列表 `warns` 已隔离, stdlib `warnings` 模块未遮蔽", None)


def check_no_std_key_typo():
    """checkable 22: profile_data.py 不应再用 `m.get("std",` (键名错).

    背景: v1.5.7.6 审计发现 profile_data.py L435 `m.get("std", 0)` 键名错.
    `_profile_continuous()` 实际写入的键名是 "sd" 不是 "std", 所以时序
    单调性检测永远 sd=0, 静默失效. v1.5.7.6 修复: `m.get("sd", 0)`.
    此 check 防回退.
    """
    target = SCRIPTS_DIR / "profile_data.py"
    if not target.exists():
    ...
    try:
        ...
        return ("red", f"读 {target.name} 失败: {e}", "查文件权限/编码")
    # 匹配 `.get("std",` (注意: 只查键名, 不查 m.get('std') 等其他形式)
    bad = list(re.finditer(r'\.get\(\s*[\'"]std[\'"]\s*,', content))
    if bad:
        line_nos = [content[:m.start()].count("\n") + 1 for m in bad]
        return ("red",
                f"profile_data.py L{line_nos} 用 `.get(\"std\", ...)` 取键错名 (_profile_continuous 实际写 'sd'), 时序检测永远返回 0",
                f"把 `.get(\"std\", 0)` 改成 `.get(\"sd\", 0)`. 命中行: {line_nos[:5]}")
    return ("green", "profile_data.py 取 sd 键名正确, 时序单调性检测可用", None)


def check_no_sys_exit_in_aigc():
    """checkable 23: aigc_scan.py 库函数不应再 `sys.exit` (会杀进程).

    背景: v1.5.7.6 审计发现 aigc_scan.py L119 `load_text()` 文件不存在时调
    `sys.exit(1)`. 作为库被其他脚本 import 时, sys.exit 会杀死整个调用
    进程, 不可恢复. v1.5.7.6 修复: 改 `raise FileNotFoundError(...)`.
    此 check 防回退.
    """
    target = SCRIPTS_DIR / "aigc_scan.py"
    if not target.exists():
        return ("yellow", f"{target.name} 不存在, 跳过", "检 skill 完整性")
    try:
        content = target.read_text(encoding="utf-8", errors="replace")
        ...
        return ("red", f"读 {target.name} 失败: {e}", "查文件权限/编码")
    bad = list(re.finditer(r"^\s*sys\.exit\s*\(", content, re.MULTILINE))
    if bad:
        line_nos = [content[:m.start()].count("\n") + 1 for m in bad]
        return ("red",
                f"aigc_scan.py L{line_nos} 调 `sys.exit(...)`, 作为库 import 时会杀进程, 不可恢复",
                f"改成 `raise FileNotFoundError(...)` 或 `raise ValueError(...)`, 让调用方 try/except 处理. 命中行: {line_nos[:5]}")
    return ("green", "aigc_scan.py 不再调 sys.exit, 可安全作为库 import", None)


def check_v158_translator():
    """checkable 24: references/读题逐句翻译.md 存在 (v1.5.8 BZD 借鉴).

    背景: v1.5.8 借鉴 BZD `bzd-problem-translator` v1.0 (2026-08-18) 的方法论
    (语义翻译 + 句子单元 + 覆盖台账 + 隐藏约束 + Mermaid 跨题流程图).
    按 v1.5.1 精简风格搬运到 references/读题逐句翻译.md, 不复制完整 BZD 子树.
    此 check 防 v1.5.8 借鉴丢失.
    """
    target = REFS_DIR / "读题逐句翻译.md"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.8 BZD 借鉴必出)",
                "从 BZD 数模社 bzd-problem-translator 借鉴, 写 references/读题逐句翻译.md (语义翻译 + 覆盖台账 + 隐藏约束 + Mermaid)")
    return ("green", f"{target.name} 存在, v1.5.8 BZD 借鉴完整", None)


def check_v158_ideas():
    """checkable 25: references/建模方案框架.md 存在 (v1.5.8 BZD 借鉴).

    背景: v1.5.8 借鉴 BZD `bzd-modeling-ideas` v1.0 (2026-08-18) 的方法论
    (整题视角 + 跨题连续性 + 候选模型比选 + 创新点 + 验证闭环).
    按 v1.5.1 精简风格搬运, 不复制完整 BZD 子树. 此 check 防丢失.
    """
    target = REFS_DIR / "建模方案框架.md"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.8 BZD 借鉴必出)",
                "从 BZD 数模社 bzd-modeling-ideas 借鉴, 写 references/建模方案框架.md (整题方案 + 跨题连续性 + 模型比选)")
    return ("green", f"{target.name} 存在, v1.5.8 BZD 借鉴完整", None)


def check_v158_workflow():
    """checkable 26: references/端到端工作流对照.md 存在 (v1.5.8 BZD 借鉴).

    背景: v1.5.8 借鉴 BZD `bzd-modeling-workflow` v1.0 (2026-08-18) 的总控方法论
    (阶段识别 + 路由规则 + 产出传递 + 下一步建议).
    按 v1.5.1 精简风格搬运, 做 BZD ↔ li-mtrie 5 步状态机对照表.
    此 check 防丢失.
    """
    target = REFS_DIR / "端到端工作流对照.md"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.8 BZD 借鉴必出)",
                "从 BZD 数模社 bzd-modeling-workflow 借鉴, 写 references/端到端工作流对照.md (BZD ↔ li-mtrie 阶段对照)")
    return ("green", f"{target.name} 存在, v1.5.8 BZD 借鉴完整", None)


def check_v158_school_awards():
    """checkable 27: references/学校画像与备赛评估.md 存在 (v1.5.8 BZD 借鉴).

    背景: v1.5.8 借鉴 BZD `bzd-cumcm-school-awards` v1.0 (2026-08-18) 的方法论
    (5 年榜单查询 + 学校画像 4 类校 + 省/国奖距离评估 + 6.81% 概率口径 + 备赛 4 档).
    按 v1.5.1 精简风格搬运, 不复制 BZD 的 assets/ 学校榜单和 scripts/query_school.py.
    此 check 防丢失.
    """
    target = REFS_DIR / "学校画像与备赛评估.md"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.8 BZD 借鉴必出)",
                "从 BZD 数模社 bzd-cumcm-school-awards 借鉴, 写 references/学校画像与备赛评估.md (4 类校 + 4 档备赛)")
    return ("green", f"{target.name} 存在, v1.5.8 BZD 借鉴完整", None)


def check_v1581_llm_translator():
    """checkable 28: references/llm-prompts/06-读题翻译.md 存在 (v1.5.8.1 BZD 借鉴配套).

    背景: v1.5.8 借鉴 BZD `bzd-problem-translator` 的方法论写 references/读题逐句翻译.md,
    v1.5.8.1 补 LLM 工具版本 (配套 prompt) 到 references/llm-prompts/06-读题翻译.md.
    此 check 防 v1.5.8.1 借鉴丢失.
    """
    target = LLM_PROMPTS_DIR / "06-读题翻译.md"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.8.1 BZD 借鉴配套必出)",
                "写 references/llm-prompts/06-读题翻译.md (4 节翻译报告 + Mermaid 跨题流程图 prompt)")
    return ("green", f"{target.name} 存在, v1.5.8.1 配套完整", None)


def check_v1581_llm_ideas():
    """checkable 29: references/llm-prompts/07-建模方案.md 存在 (v1.5.8.1 BZD 借鉴配套).

    背景: v1.5.8 借鉴 BZD `bzd-modeling-ideas` 的方法论写 references/建模方案框架.md,
    v1.5.8.1 补 LLM 工具版本 (配套 prompt) 到 references/llm-prompts/07-建模方案.md.
    此 check 防 v1.5.8.1 借鉴丢失.
    """
    target = LLM_PROMPTS_DIR / "07-建模方案.md"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.8.1 BZD 借鉴配套必出)",
                "写 references/llm-prompts/07-建模方案.md (5 节建模方案 + 模型比选 + 跨题连续性 prompt)")
    return ("green", f"{target.name} 存在, v1.5.8.1 配套完整", None)


def check_v159_aigc_audit_prompt():
    """checkable 30: references/llm-prompts/08-AIGC审计.md 存在 (v1.5.9 BZD v2.2 加深借鉴).

    背景: v1.5.3 借鉴 BZD `bzd-paper-aigc-auditor` v1.0 的两层审计结构 (60%+40%),
    但没借鉴 LLM agent 完整 prompt + HTML 报告. v1.5.9 借 v2.2 加深借鉴, 写
    references/llm-prompts/08-AIGC审计.md (两层审计 + 9 维 + 6 类 + HTML 报告 prompt).
    此 check 防 v1.5.9 加深借鉴丢失.
    """
    target = LLM_PROMPTS_DIR / "08-AIGC审计.md"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.9 BZD 加深借鉴必出)",
                "从 BZD 数模社 bzd-paper-aigc-auditor v2.2 加深借鉴, 写 references/llm-prompts/08-AIGC审计.md (两层审计 + 9 维 + 6 类 + HTML 报告 prompt)")
    return ("green", f"{target.name} 存在, v1.5.9 加深借鉴完整", None)


def check_v159_aigc_audit_html():
    """checkable 31: references/审计报告模板.html 存在 (v1.5.9 BZD 加深借鉴配套).

    背景: v1.5.9 借 BZD `bzd-paper-aigc-auditor` v2.2 的 HTML 分层报告结构,
    自己重写简化版 (li-mtrie 原创, 不复制 BZD 版权 HTML) 到 references/审计报告模板.html.
    学生用后输出 AIGC 审计报告 (浏览器打开填占位符).
    此 check 防 v1.5.9 配套 HTML 模板丢失.
    """
    target = REFS_DIR / "审计报告模板.html"
    if not target.exists():
        return ("red", f"{target.name} 缺失 (v1.5.9 BZD 加深借鉴配套必出)",
                "从 BZD bzd-paper-aigc-auditor v2.2 HTML 报告结构借鉴, 自己重写简化版到 references/审计报告模板.html (li-mtrie 原创, 不复制 BZD 版权)")
    return ("green", f"{target.name} 存在, v1.5.9 配套 HTML 模板完整", None)
...
CHECKS = [
    ...
    ("21. profile_data.py 不遮蔽 warnings 模块 (v1.5.7.6 审计 fix)", check_no_warnings_shadow),
    ("22. profile_data.py sd 键名正确 (v1.5.7.6 审计 fix)", check_no_std_key_typo),
    ("23. aigc_scan.py 库函数不再 sys.exit (v1.5.7.6 审计 fix)", check_no_sys_exit_in_aigc),
    ("24. v1.5.8 BZD 借鉴 读题逐句翻译.md (借 bzd-problem-translator)", check_v158_translator),
    ("25. v1.5.8 BZD 借鉴 建模方案框架.md (借 bzd-modeling-ideas)", check_v158_ideas),
    ("26. v1.5.8 BZD 借鉴 端到端工作流对照.md (借 bzd-modeling-workflow)", check_v158_workflow),
    ("27. v1.5.8 BZD 借鉴 学校画像与备赛评估.md (借 bzd-cumcm-school-awards)", check_v158_school_awards),
    ("28. v1.5.8.1 BZD 配套 06-读题翻译.md (借 bzd-problem-translator LLM 版)", check_v1581_llm_translator),
    ("29. v1.5.8.1 BZD 配套 07-建模方案.md (借 bzd-modeling-ideas LLM 版)", check_v1581_llm_ideas),
    ("30. v1.5.9 BZD 加深 08-AIGC审计.md (借 bzd-paper-aigc-auditor v2.2 LLM agent)", check_v159_aigc_audit_prompt),
    ("31. v1.5.9 BZD 加深 审计报告模板.html (借 bzd-paper-aigc-auditor v2.2 HTML 结构)", check_v159_aigc_audit_html),
]
...
def main():
    ...
    if json_mode:
        ...
        print("=" * 60)
        print("li-mtrie-2026 赛前 1 天 + CI smoke-test (v1.5.9)")
        print("=" * 60)
        green_count = 0
        for name, info in results.items():
            sym = {"green": "🟢", "yellow": "🟡", "red": "🔴"}.get(info["status"], "?")
