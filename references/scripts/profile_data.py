...
工作流位置:
    用户给数据
        ...
        → code-template.py 顶部 matplotlib rcParams 配置字体
        → plot
        → visual_qa.py 程序自检 + AI 读图（PNG 预览）
        → check_figure.py
...
def profile_data(source, group_cols: list[str] | None = None) -> dict:
    ...
    warns: list[str] = []  # 局部变量, 避免遮蔽 stdlib `warnings` 模块
    for c in df.columns:
        s = df[c]
    ...
    info = {
        ...
        "warnings": warns,  # 保留 key 名以兼容 JSON 输出
    }
...
def _detect_time_series(cols: dict, cont: list[str]) -> str | None:
    ...
    for col_name in cont:
        ...
        if m["type"] == TYPE_CONTINUOUS and m.get("n_total", 0) > 5:
            # 简化判断: 提取列的 summary
            # 注意: _profile_continuous() 写入的键名是 "sd" 不是 "std"
            sd = m.get("sd", 0)
            mean = m.get("mean", 0)
            mn = m.get("min", 0)
            mx = m.get("max", 0)
...
def _cli() -> int:
    ...
    if is_excel and args.all_sheets:
        ...
        for sn in xl.sheet_names:
            ...
            df = pd.read_excel(args.source, sheet_name=sn, header=0)  # 用 Excel 默认表头
            # 保存为临时 csv 调 profile_data (保留表头)
            import tempfile, os
            with tempfile.NamedTemporaryFile(suffix='.csv', delete=False, mode='w', encoding='utf-8') as f:
                df.to_csv(f.name, index=False, header=True)
                tmp_path = f.name
            try:
