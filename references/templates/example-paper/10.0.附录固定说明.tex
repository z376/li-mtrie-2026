...
% ====== 附录 C-F: 各问题源程序说明模板 (v1.5.7.3 通用化) ======
% ====== 增删说明 (跑题用户必读) ======
% 1. 赛题有几问就复制几个 \subsection*{附录 X \quad 问题 X 完整源程序}, 编号连续 (C/D/E/F/G/...)
%    - 2 道题: 删附录 E / F, 保留 C/D
%    - 3 道题: 删附录 F, 保留 C/D/E
%    - 4 道题 (本模板默认): C/D/E/F 4 块, 全部保留
%    - 5 道题: 复制附录 F 改名附录 G, 程序名 5_eee.py
%    - 6+ 道题: 以此类推
% 2. 每块内容 (8 行 itemize + 1 段 verbatim 代码) 来自本模板, 改 5 处:
%    - 程序名称 (问题 X_yyy.py)
%    - 对应问题 (问题 X)
%    - 输入/输出文件 (填自己实际路径)
%    - 依赖环境 (填自己实际 import)
%    - 正文对应位置 (填自己第 X 章第 X 节)
% 3. verbatim 代码块: 贴 30-50 行核心代码即可, 完整代码放 支撑材料/ 程序使用说明
% 4. 评阅要求: 必须跟 §5 正文一一对应, 不能空泛列项
% ====== 增删说明结束 ======
