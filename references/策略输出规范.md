# 策略输出规范 (v1.5.0 借鉴 bzd-modeling-ideas v1.0, 原文英文 + 中文要点)

> 借鉴自 `bzd-modeling-ideas` skill (v1.0, 2026) `references/output-standard.md` (英文, 国际版),
> 本文件保留英文原文 + 中文要点提炼, 供学生 + 评审双向参考.

## Concise question statement (题目概述 4 字段)

For each question state:

- `任务`: action and object, not copied prose; (动作 + 对象, 不抄题面)
- `已知`: supplied conditions and inherited outputs only; (题面条件 + 前问输出)
- `输出`: exact numerical, categorical, functional, graphical, or file deliverable; (数值/类别/函数/图/文件)
- `本质`: mathematical structure plus justification. (数学结构 + 选型理由)

Never mix added assumptions into known conditions. (绝不把假设混入已知条件)

## Required per-question sequence (每问 4 段式硬约束)
