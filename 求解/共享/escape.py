...
def time_varying_dijkstra(G, points, segments, t_arr, t_full,
                          ...
                          max_steps=7):
    """时变 Dijkstra: 工人从 worker_xyz 出发, 找每个 exit 的最短到达时间
    ...
    Returns:
        dict {exit_xyz: (arrive_time, path_points_list)}
    """
    # Snap 工人到最近巷道
    ...
    # Dijkstra: 状态 = (端点, 时刻, 步数)
    # 边权: 进入巷道 k 时的水深决定 V, 通过时间 = L/V
    # 注意: 走 snap 段 (虚拟源到 pa_w/pb_w) 时, 假设无水 (按"工人起步瞬间 t=1min, 水可能已到")

    INF = math.inf
    dist = {VIRT_W: t_start}
    prev = {}  # (node, t) -> (prev_node, prev_t, via_seg)
    pq = [(t_start, 0, VIRT_W)]  # (time, steps, node)
    step_count = {VIRT_W: 0}
    ...
    while pq:
        ...
        for v in G2.neighbors(u):
            ...
            if u == VIRT_W:
                ...
                # 正常巷道: t_arr 端是 u 还是 v 决定水流方向
                # 工人从 u 走向 v, t_arr[u] vs t_arr[v] 决定顺逆
                ta = t_arr.get(u)
                tb = t_arr.get(v)
                if ta is None or tb is None:
                    depth = 0.0
                    ...
                    # 水流: t_arr 小→大. 工人从 u→v.
                    if ta < tb:
                        # 水从 u→v 方向, 工人也 u→v = 顺水
                        direction = 'down'
                    elif ta > tb:
                        direction = 'up'  # 工人逆水
                    else:
                        direction = 'dry'
                    # t_full 是巷道 (u, v) 的充满时刻
                    ...
                    # 水深: 工人进入时刻取"到达 v 端的时刻" 还是 "进入 u 端的时刻"?
                    # 简化: 用到达 v 端的时刻 (更严, 离开时水深)
                    # 但通过时间是 L/V, 实际水深在 [t_enter, t_enter + L/V] 内线性变化
                    # 简化: 用中点水深
                    t_enter = t_now
                    t_exit = t_now + L / V_DRY_MIN  # 先估一个
    ...
    for V, exit_xyz, snap_xyz_e in virt_exits:
        if V in dist:
            results[exit_xyz] = (dist[V], [])
        else:
            results[exit_xyz] = (None, [])
    return results
