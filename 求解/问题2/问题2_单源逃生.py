...
输入: Q1 的水流到达/充满时刻
输出: 3 工人最佳逃生路径 → result2-1.xlsx, result2-2.xlsx
"""
import os
import sys
import time
...
def draw_escape_paths_3d(G, worker_paths, out_path, title):
    """画 3D 网络 + 3 工人逃生路径"""
    ...
    for i, (worker_xyz, path) in enumerate(worker_paths):
        ...
        ax.plot(px, py, pz, color=colors[i], linewidth=2.0, marker='.',
...
def draw_worker_time_comparison(worker_results, out_path, title):
    """画 3 工人到达 2 出口的时间对比柱状图"""
...
def main():
    t_total = time.time()
    for attach_n, source_xyz, exits, workers, suffix in [
        (1, SOURCE_A1,
         [EXIT_1_1, EXIT_1_2],
        ...
        # 跑 Q1 水流
        flow_res = run_single_source(G, points, segments, source_xyz, 'A')
        t_arr = flow_res['t_arr']
        t_full = flow_res['t_full']
        print(f'  Q1 水流: t_arr 已算, 663 端点可达')

        # 对每个工人跑逃生
        worker_results = []
        worker_paths = []  # for 3D 画图
        for w_idx, worker_xyz in enumerate(workers):
            ...
            res = time_varying_dijkstra(G, points, segments, t_arr, t_full,
                                         worker_xyz, exits, t_start=T_ALARM)
            # 选最佳出口 (最早到达)
            best_exit = None
            best_time = None
            best_path = []
            ...
            if best_path:
                print(f'    路径 {len(best_path)} 步, 首步 {best_path[0][:4]}, 末步 {best_path[-1][:4]}')
        ...
        # 图 1: 3D 逃生路径
        draw_escape_paths_3d(G, worker_paths,
                             os.path.join('图片', f'问题2_附件{attach_n}_3D逃生.png'),
                             f'Q2 附件{attach_n}: 3 工人逃生路径 (单源水流)')

        # 图 2: 时间对比
        draw_worker_time_comparison(worker_results,
                                    os.path.join('图片', f'问题2_附件{attach_n}_时间对比.png'),
                                    f'Q2 附件{attach_n}: 3 工人到达 2 出口时间对比')
