...
| 规范版本 | 适配情况 | 关键差异点 |
|---------|---------|-----------|
| **2026（默认）** | ✅ 完全适配 | 摘要中文、**正文 ≤ 30 页 (附录不计入)**、附录两段固定说明、AI 声明在参考文献之前、详情 PDF 在支撑材料 |
...
**踩坑案例** (2024 调度类测试): 没装 pulp → 重茬约束解不了; 赛场断网 PyMuPDF wheel 拉不下 = 一夜没装上包.
...
**📌 5 道防线速查表 (v1.5.7.11 新增, 必读中心索引)**:

| 防线 | 触发场景 | 核心规则 | 对应文档 | dryrun check |
|------|---------|---------|---------|------------|
...
**详细反模式对照表 + 跑题前/后自检 checklist** → `references/5道防线自检清单.md` (v1.5.7.11 新增, 6.5 KB, 一处调取). dryrun **check 24** 验证该文件存在 + 4 章节齐全.
...
| 12 | **二选一: 选题** | 题面"本题号" | 本科 ABC (不是 D/E 高职) | 选题决策 |
...
**📌 0:00 信息边界原则 (v1.5.7.6 新增, 防数据泄露)** — **15 项必填之前必先做**:

数据泄露是历年 CUMCM 最大踩坑: 0:00 制定计划时**只能**基于已知信息 (历史数据 / 预报), **不能**用当天及以后实际数据 (评阅要点对应章节 "数据使用" 条款 (每年 4 月公布, 跑题前查最新), 包括隐含使用). 跑题前必须列出三类信息边界:

| 信息类别 | 0:00 时已知? | 数据源 | 用途 |
|---------|------------|--------|------|
| **历史数据** (昨天/上周/历史平均) | ✓ 已知 | 附件 N 全年实际 row [day_idx-1] 等 | **0:00 预测输入** |
| **当天及以后实际** | ✗ 未知 | 附件 N row [day_idx] | **执行阶段 + 紧急购电对比基准** |
| **未来预报** (PV/负载预报) | ✓ 已知 (发布) | 附件 M 预报 | **0:00 预测 + 滚动调整输入** |

**判定规则**: 任何 0:00 LP 求解, 预测输入必须是"历史数据" 或"未来预报", **绝对不能**是"当天实际". 紧急购电/调整偏差**只能在**"执行阶段 vs 0:00 计划" 的对比中产生, 不是 0:00 LP 内部最小化目标.

**反模式 (踩过的坑, v1.5.7.6 实战案例)**:
1. ❌ 0:00 用附件 N 当天实际数据 (附件 2 row day_idx) 做 LP → 数据泄露 (隐含用未来信息)
2. ❌ 缩 g 上界 / 人造阴雨 / 改 LP 目标让紧急购电"非 0" → 物理造假, 不是题面要求
3. ❌ 用附件 M (Q3 预报) 做 Q2 → 越界, 题面规定 Q2 用 N+Q
4. ✅ 0:00 用前一天实际 / 历史平均 / 类别平均 (周末 vs 工作日) → 数据隔离正确, 紧急购电自然由"预测误差"触发

详细原则 + 实战案例 → `references/信息边界原则.md`. dryrun check 21 自动扫描 .py 检测 "day_idx 当天实际" 误用 → RED.

**📌 题目设计意图分析 (v1.5.7.7 新增, 防"按字面跑不解题")** — **15 项 + 信息边界之后必做**:

CUMCM 题目是**研究性问题**, 不是"按字面跑 LP". 题目设计有**递进关系**和**隐含期望方向**, 跑完后**必须验证期望**, 不达预期就要回头改方法. 我之前测 skill 只看"流程跑通", 没看"解题方法是否对" — **Q3 紧急购电应 < Q2** (滚动调整应压缩紧急购电), 但我跑出来 Q3 (362/365, e=329.7 万) 比 Q2 (315/334, e=259.0 万) **更多**, 说明**方法错或预报精度不足**, 必须回头改. 跑题前先识别 3 件事:

1. **递进关系**: N 个问题间的逻辑链条 (e.g. Q2 提出机制 → Q3 优化机制 → Q4 加维度)
2. **每个问题测什么**: e.g. Q2 测"无预警场景 + 紧急购电代价", Q3 测"滚动调整 + 双向偏差 vs 紧急购电 5x 净节省"
4. **期望方向**: 跑完对照期望, 不达预期 = 方法错, 回头改

| 题目设计 | Q1 | Q2 | Q3 | Q4 |
|---------|----|----|----|----|
| **测什么** | 单日 LP 基线 | 紧急购电 5x 代价 | **滚动调整压缩紧急购电** | 波动电价下避免高电价紧急购电 |
| **递进** | 基础 | **提出机制** | **优化机制** | 加波动电价维度 |
| **期望方向** | 总费最优 | 紧急购电 > 0 (物理真实) | **紧急购电 ↓** | **紧急购电 ↓ 或转移** |

**实战复盘 (2026 C)**:
- Q3 紧急购电 329.7 万 kWh > Q2 259.0 万 kWh — **方向错, 说明滚动调整没起到压缩作用**, 附件 3 预报误差累积触发更多紧急购电. 正确做法可能是: ① 用**前一天实际** 替代附件 3 0:00 预报做 0:00 计划 (附件 3 误差更大), ② **减少滚动次数** (只在 12:00 一次调整, 不 6/12/18 三次)
- Q4-3 紧急购电 333.8 万 kWh 跟 Q3 329.7 万 kWh **基本相同** — 波动电价**没让储能避免高价时段紧急购电**, 套利策略需优化 (e.g. 高电价时段前**预防性预充**, 让 SOC 满, 避免紧急购电)
...
**📌 预测方法选择 (v1.5.7.6 新增, 防 0:00 数据泄露)** — 题目说 "0:00 制定当天计划" 时:

| 预测方法 | 数据源 | 适用 | 精度 | 计算量 |
|---------|-------|------|------|--------|
| **前一天实际** | 附件 N row [day_idx-1] | 简单稳定场景 | 中 (相邻天相近) | 极低 |
| **N 天移动平均** | row [day_idx-N:day_idx-1] | 中期稳定 | 中-高 | 低 |
| **同类型日平均** (§2.4 评阅要点) | 上周同 weekday/weekend 类日 | 周期性场景 | 高 | 低 |
| **ARIMA / GM(1,1)** | 历史时间序列 | 长期趋势 | 中-高 | 中 |
| **附件 M 预报 (Q3)** | row [day_idx, time_idx] | 实时滚动 | 高 | 中 |

**选择规则**: 题目要求"0:00 制定" + 有"已知数据" (附件 N 历史) → 选**前一天实际**或**同类型日平均**; 题目要求"滚动调整" + 有附件 M 预报 → 选**附件 M 预报 + 偏差分析**.

**强制要求 (§2.4 评阅要点)**: 负载预测必须**分类** (e.g. 周五周六 vs 周日至周四), 不分类会扣"未充分数据分析"分.

**📌 递进关系分析 (v1.5.7.7 新增, 防"按字面跑不解题")**:

跑题前**必做** 1 件事: **识别 N 个问题间的递进关系 + 每个问题测什么 + 期望结果方向**. 这是 调度类实战暴露的关键盲点 — 我之前 Q3 跑出紧急购电**比 Q2 还多**, 因为没识别 "Q3 应**压缩**紧急购电" 这个期望, 跑完没回头.

**4 步法**:
1. **画递进图**: N 个问题间的逻辑链 (e.g. Q1 基线 → Q2 提出紧急购电 → Q3 滚动调整压缩紧急购电 → Q4 加波动电价维度)
2. **每个问题测什么**: 1-2 句话概括 (e.g. Q2 = "无预报场景, 紧急购电 5x 代价"; Q3 = "滚动调整应**压缩**紧急购电, 净收益 = 5x - 1.5x = 3.5x/kWh 节省")
3. **期望方向**: 列每个问题的**关键指标**期望 (e.g. Q3 e < Q2, Q3 调整偏差 < Q2 紧急购电费)
4. **跑完验证**: 不达预期 = 方法错, 回头改方法 (e.g. 换预测数据源, 减少滚动次数, 改套利策略)
...
## 递进关系分析
- Q1 → Q2: 单日 → 全年 + 紧急购电机制
- Q2 → Q3: 无滚动 → 4 时点滚动 (期望 Q3 紧急购电 < Q2)
- Q3 → Q4: 固定电价 → 波动电价 (期望 Q4-3 紧急购电转移到低价时段)

## 每个问题测什么
- Q1: 单日 LP 基线 (验证模型正确性)
- Q2: 紧急购电 5x 触发代价 (历史预测的误差累积)
- Q3: 滚动调整压缩紧急购电的能力 (4 时点 MPC + 双向偏差)
- Q4: 波动电价下储能套利 + 避免高价紧急购电

## 期望方向 (跑完对照)
- Q3 紧急购电天数应 < Q2
- Q3 调整偏差费应 < Q2 紧急购电费 (净收益)
- Q4-3 紧急购电应转移到低价时段 (时段分布变化)
```
...
| PowerShell `Compress-Archive` | `Compress-Archive -Path 求解\* -DestinationPath 支撑材料.zip -Force` | `.zip`（**不是真 .rar, 仅兜底**） | ⭐⭐ 兜底用 |
