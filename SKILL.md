...
**本 skill 所有 step 完成判据遵循 `checkable` 原则** — 完成状态由 1 行命令或 1 次肉眼扫描验证为 **`green`/`red` 二元** (用 `green`/`red` 而非"绿/红", 锚定 pretrained 视觉 prior), 不接受"应该差不多了"。**red = 立即停下修, `green` = 跳下一步**。终态: 5 步全 `green` = **`sign-off`** = 可交卷。
...
**📌 Step 0 必填 15 项读题清单 (v1.5.7 新增, 防 2025C 3 P0 错位)**:
...
| 6 | **关键判据 (异常)** | 题面"\|Z\| > 3 异常" | 13/18/21 任一 \|Z\|>3 异常 | 问题 4 |
| 7 | **分组标准 (BMI)** | 题面"[20,28) [28,32) [32,36) [36,40) ≥40" | 5 组左闭右开 (pd.cut right=False) | 问题 2/3 |
| 8 | **时间窗口** | 题面"早期 12 周 / 中期 13-27 周 / 晚期 28 周+" | 3 阶段 | 风险评估 |
| 9 | **风险权重** | 题面"漏检严重 / 过度诊断轻" | 2:1 临床损失 | 贝叶斯决策 |
| 10 | **输出形式** | 题面"推荐最佳检测孕周 + 风险评估" | 孕周数 + 风险等级 | 结果呈现 |
| 11 | **样本量约束** | 题面"1082 例男胎 / 605 例女胎" | 跨问题不混用 | 数据加载 |
| 12 | **二选一: 选题** | 题面"C 题" | C 题 (不是 D/E) | 选题决策 |
| 13 | **二选一: AI 声明** | 题面"使用/未使用 AI 工具" | 二者择一 | 9.0 声明 |
| 14 | **附录必填** | 题面"附录含支撑材料清单 + 源程序" | A 清单 + B 环境 + C-F 各问题代码 | 10.0 附录 |
| 15 | **隐藏约束** | 题面"未明确说但需推断" | 例: 数据缺失/异常处理原则 | 数据预处理 |

**为啥 15 项全覆盖**: 2025C 跑题 3 P0 错 (问题 4 sheet 错 + 问题 2/3 BMI 自创 + 附录残留) 全部源自**没读完题就跑题**。15 项强制锚定让 every 跑题用户必须先 answer "题面说啥" 才能开始建模, 防 other agent 跳过题面理解直接抄模板。
...
**Step 3 起手必做 3 件**（漏写就 0 分，2026 规范硬性要求）：

1. **AI 工具使用声明**（位置：`论文/9.0.AI工具使用声明.tex`，紧邻参考文献之前）
   - **§9 论文内 = 1 句话简短声明**（含 3-5 项简要用途），**不要把详细 5 项/4 节写这里**
   - 详细见第 3 件
2. **附录两段固定说明**（位置：`论文/10.0.附录固定说明.tex`，附录 1 程序 + 附录 2 支撑材料）
3. **AI 工具使用详情.pdf**（如果用了 AI, **必填**, 2026 AI 规定第 4 条硬性要求）：
   - 位置：`论文/AI工具使用详情.pdf` → 复制到 `支撑材料/AI工具使用详情.pdf`（**不是附录**）
   - **4 节详写**: 工具信息 / 使用目的与环节 / 主要 prompt 与过程 / 核验情况
   - 模板: `references/templates/example-paper/AI工具使用详情.tex`
   - **跟 §9 1 句话的分工**: §9 简要说"用了 AI 干什么" (3-5 项 简要), AI 详情 PDF 详写"每条 prompt + AI 响应 + 采纳修改 + 核验" (4 节)

> AI 工具使用声明 = **§9 1 句话 (3-5 项简要) + AI 详情 PDF (4 节详写)** + 6 项 AI 必查 (见下) → 见 `references/合规检查清单.md §1.3`（AI 工具类）。

**📌 每章写完查 板块自查（v1.5.3 借鉴 BZD 12 子 skill, 9 板块级, 必做）**:
...
- `支撑材料/AI工具使用详情.pdf` — 用 AI 必填（2026 AI 规定第 4 条）
  - **4 节**: 工具信息 / 使用目的与环节 / 主要 prompt 与过程 / 核验情况
  - 模板: `references/templates/example-paper/AI工具使用详情.tex`
