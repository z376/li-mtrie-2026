...
数据泄露是历年 CUMCM 最大踩坑: 0:00 制定计划时**只能**基于已知信息 (历史数据 / 预报), **不能**用当天及以后实际数据 (评阅要点 §2.6 明确禁止, 包括隐含使用). 跑题前必须列出三类信息边界:
...
CUMCM 题目是**研究性问题**, 不是"按字面跑 LP". 题目设计有**递进关系**和**隐含期望方向**, 跑完后**必须验证期望**, 不达预期就要回头改方法. 我之前测 skill 只看"流程跑通", 没看"解题方法是否对" — **Q3 紧急购电应 < Q2** (滚动调整应压缩紧急购电), 但我跑出来 Q3 (362/365, e=329.7万) 比 Q2 (315/334, e=259.0万) **更多**, 说明**方法错或预报精度不足**, 必须回头改. 跑题前先识别 3 件事:
...
跑题前**必做** 1 件事: **识别 N 个问题间的递进关系 + 每个问题测什么 + 期望结果方向**. 这是 2026 C 题实战暴露的关键盲点 — 我之前 Q3 跑出紧急购电**比 Q2 还多**, 因为没识别 "Q3 应**压缩**紧急购电" 这个期望, 跑完没回头.
