...
description: |
  ...
  并说"开始跑题"或"读题翻译"/"建模方案"，求解后说"出论文 PDF"，或验证本 skill ("smoke test")。
...
**本 skill 所有 step 完成判据遵循 `checkable` 原则** — `checkable` = 每个 step 有 1 行命令或 1 次肉眼扫描可验证为 **`green`/`red` 二元** (用 `green`/`red` 而非"绿/红", 锚定 pretrained 视觉 prior), 不接受"应该差不多了"。**red = 立即停下修, `green` = 跳下一步**。终态: 5 步全 `green` = **`sign-off`**。
...
3. LaTeX 模板的 `论文/format.cls` 一般无需改（`\geometry` 边距等参数 2024/2026 一致；模板源在 `references/templates/example-paper/`，跑题时复制到 `论文/`）
...
如果用户只说"开始跑题"而没放题/数据，**必须**先停下来等用户放好再启动。
...
**赛前 1 天 dry-run 自动化** (v1.5.2 新增, v1.5.7 升级到 20 项, v1.5.7.5 扩到 23 项): 跑 `python references/scripts/dryrun.py` 一键验证 23 项 checkable:
装包 / Python 脚本 / LaTeX 编译 / Overfull 数 / pack.py / AIGC 风险 / SKILL.md frontmatter / LLM 工具 / BZD 借鉴 / 拟合兼容 / LICENSE / 5 步状态机 / LaTeX 可执行性 / 占位符 / 附录文件 / 图引用 / Post-Solution Audit / **读题清单 (v1.5.7 新增)** / **v1.5.7.5 修剪 4 项 (description/红线/CHANGELOG/4 完成判据)** / **v1.5.7.5 审计 fix 3 项 (warnings 遮蔽/sd 键名/sys.exit)**. **23/23 全 green = sign-off 绿, 可安心参赛**. 跟 CI `smoke-test.yml` 等价, 但本地手动跑. 出 red 项时脚本给修复建议.
...
│  → sign-off (全 `green` = 可交卷, 缺任一项 = `red` = 立即修) │
└──────────────────────────────────────────────────────────┘
```
...
1. **题面 quote 锚定** — 每问 1 行, **原 quote + 解读** 写到 `求解/求解计划.md` 顶部 quote 表:
   ...
   **为啥必做**: 2025C 跑题 other agent 把问题 4 答成"判男胎异常"，整问白做。quote 表让每问跟原文锚定。
...
2. **题面关键判据 grep** — 用 `grep -oP '[\d.]+%|≥?\s*[\d.]+|≤?\s*[\d.]+|\|Z\|>[\d.]+' 题目/C题.pdf` 提取题面所有数字判据 (4% / |Z|>3 / BMI 边界 / 风险权重), 写到 `求解/求解计划.md` "题面约束" 段:
   ...
   **为啥必做**: 2025C 问题 2/3 自创 BMI 分组跟题面 [20,28)/[28,32)/[32,36)/[36,40)/≥40 完全不同，评审判"题意理解不深"。先 grep 提取，跑题时逐步对照。

**📌 Step 0 必填 15 项读题清单 (防 2025C 3 P0 错位)**:

跑题前**必须**写完 `求解/读题清单.md` (15 项全覆盖, `references/读题清单.md` 提供模板 + 进阶追问), 缺一项 dryrun check 19 直接 RED:

| # | 项 | 题面原话 quote | 你的解读 | 对应代码/论文 |
|---|----|----------------|----------|---------------|
| 1 | **题组 + 题号** | 题面"本科组"+"C题" | 本科 C 题 (不是 D/E) | 选题决策 |
| 2 | **总问题数** | 题面"4 个问题" | 4 问 | 5.1.1-5.1.4 |
| 3 | **数据附件结构** | 题面"附件.xlsx 含 Sheet1/Sheet2" | Sheet1 男胎 1082 + Sheet2 女胎 605 | pd.read_excel |
| 4 | **每问用哪个 sheet** | 题面"问题 4 用女胎" | 问题 1-3 Sheet1, 问题 4 Sheet2 | sheet_name= |
| 5 | **关键判据 (达标阈值)** | 题面"Y 染色体浓度达到或高于 4%" | 0.04 = 4% 达标 | 问题 1/2/3 |
| 6-15 | (完整 10 项 + 进阶追问 + 字段说明见 `references/读题清单.md` 模板) | - | - | - |

**为啥 15 项全覆盖**: 2025C 跑题 3 P0 错（问题 4 sheet 错 + 问题 2/3 BMI 自创 + 附录残留）全部源自没读完题就跑题。15 项强制锚定让跑题前先 answer "题面说啥"才能开始建模。

**落地工具**:
- 模板: `references/读题清单.md` (15 项空表, 跑题用户填)
- LLM 工具: `references/llm-prompts/05-读题提取.md` (从题面 PDF 自动提取)
- 程序化检查: `references/scripts/dryrun.py` check 19 (验证清单存在 + 15 项全覆盖)
- 同步章节: 5.1.1 §5.1 写完后, 用本清单逐项对照 — 一致才进 Step 2 求解

**警示**: 15 项中任一项没填, dryrun check 19 RED, sign-off 不过 (跟 check 15/16/17 一致, 模板状态 yellow, 跑题用户 RED)。
...
**⚠️ 4 大典型坑 (防"答非所问")** — 跑题前必读, 都源自 2025C 真实失败案例:
...
| **pd.read_excel 选错 sheet** | 题面提到女胎但代码用 sheet_name=0 (男胎) | **Step 0 quote 表写明每问用哪个 sheet**, 跑题前对照 pd.read_excel 的 sheet_name 参数 |
...
**候选模型池（每题型主选 + 备选）** → `references/模型决策树.md`（6 题型决策表 + 实战案例）。

速查版：

| 问题类型 | 主选 | 备选 |
|----------|------|------|
| 优化 | LP/IP 精确解 | GA / PSO / SA / ACO |
| 评价 | AHP + 熵权 | TOPSIS / CRITIC / 灰色关联 |
| 预测 | 多元回归 / RF | XGBoost / LSTM / ARIMA |
| 分类 | 逻辑回归 / RF | SVM / KNN / 朴素贝叶斯 |
| 机理 | 微分方程 / 动力学 | 元胞自动机 / Agent-Based |
...
5. **红/绿判据**:
   ...
   - **红**: 任何一环 ≥ 60 min 卡住 → **先修这一处, 修通再开始下一问**
...
> 8 项整篇一致性自检 → `references/paper-spec.md §2 8 项自检`（完整清单 + 模板 + 踩坑案例）。
...
- **统一命名**：问题代码内用同一名字指代同一个量（problem-1 用了 `PITCH`，后续**统一用大写 `PITCH`** ——评审标"模型不严谨"是给混用 `pitch` / `p` 的人）
- 每问跑完反问 3 条：
...
按论文 10 章结构对应 9 板块自查文件 (写完每章后查对应编号), 终审前按 01→09 顺序全跑:

| 章节 | 板块自查文件 | 维度 | 阈值 |
|------|-------------|------|------|
| §0 摘要 | `01-摘要自查.md` | 7 维诊断 | ≥ 8 绿 |
| §9.0 AI 声明 | `02-AI声明自查.md` | 5 问 + 4 铁律 | 4 铁律全过 |
| §1 引言 | `03-问题重述自查.md` | 3 段结构 | 全绿 |
| §2 总体分析 | `04-问题分析自查.md` | 7 维 + 4 类联动 | 全绿 |
| §3 模型假设 | `05-模型假设自查.md` | 5 维/条 | 全绿 |
| §4 符号说明 | `06-符号说明自查.md` | 7 维 | 全绿 |
| §5 建模求解 + §6 检验 | `07-模型求解自查.md` | 5 段 | 全绿 |
| §9 文献 + §10 附录 | `08-参考文献附录自查.md` | P0-P3 8 维 | 全绿 |
| 全文 AIGC | `09-AIGC审计.md` | 9+6=15 维 (60+40 分) | ≥ 80 绿 |

→ 索引 + 与现有 4 文件关系详 `references/板块自查/README.md`
...
**0 命中 = 绿**, 任 1 命中 = 立即修, 不进 Step 4. 2025C 跑题 other agent 留了 9.0 AI 声明 + 10.0 附录固定说明的【】方括号占位符, 评委直接扣分。**dryrun.py check 15 自动扫这个, 红项 = 强制 sign-off 不过**。
...
2025C 跑题 other agent 把 2024B 烟幕题的 5 个脚本写进 10.附录.tex, 但当前项目根本没有这 5 个文件 — 评委抽支撑材料目录对不上, 直接判 0 分。**dryrun.py check 16 自动扫这个, 红项 = sign-off 不过**。修法: 附录 1/2 改成你**实际**有的 py + xlsx + pdf 文件名。
...
**🟢 Step 4 完成判据**: xelatex 论文.tex/电子版.tex/AI工具使用详情.tex 各 2 遍编译绿 + 论文 ~24 页 + 电子版 ~22 页 + AI 详情 ~6 页 + 支撑材料 .rar ≤ 20MB + dryrun 23 项全绿 sign-off。

---

## 红线与失败模式 (3 类)

→ `references/红线与失败模式.md`（求解/编译失败 + 跨平台代码 + 跑题纪律 + 修复命令）。跑题遇错先查这里，跑题前过一遍 §2 跨平台红线。

---

## 合规与硬规则

→ `references/合规检查清单.md`（硬规则 + AI 合规 + 纪律红线 + 4 类检查表 + 一键验证命令）。写完论文后逐项打勾 + 跑 §2 验证命令。

---

## Output contract

→ `references/合规检查清单.md` §5 最终提交物清单（3 文件 + 支撑材料内容 + 3 项不放 + 规范第十一条）。

---

## Post-Solution Audit (跑题后必做, 4 必做 + 1 可选)

→ `references/post-solution-audit.md`（4 必做 + 1 可选 + 5 步状态机关系 + 4 个 PowerShell 验证脚本）。sign-off 绿后跑 20-30 分钟，比交卷后评委扣分划算。简述：

- 必做 1: 解题思路 vs quote 锚定表交叉验证 (防 sheet 错)
- 必做 2: 代码实际跑过 + 输出文件齐全 (py≥1, png≥4, csv≥1)
- 必做 3: 5.X.2 数字 ↔ resultX.xlsx 一致性 (防数据不可信)
- 必做 4: 5.X.2 引用的图/表都存在 (编号连续)
- 可选 5: LLM 工具 03 自动审稿 (5 维, 30 min)

## Templates inside this skill

→ `README.md` 目录结构段（与本文档保持同步，单一权威源）。

## v1.5.8 新增 4 references（BZD 借鉴 v1.5.8）

v1.5.8 借鉴 BZD 4 个 skill 的方法论 (按触发顺序排列):

| BZD 源 skill | li-mtrie references | 何时用 |
|---|---|---|
| `bzd-modeling-workflow` | `references/端到端工作流对照.md` | 跑题启动时看 BZD ↔ li-mtrie 阶段对照 (Step 0-4 总览) |
| `bzd-problem-translator` | `references/读题逐句翻译.md` | Step 0 读题后做语义翻译 + 覆盖台账 + Mermaid 跨题流程图 |
| `bzd-modeling-ideas` | `references/建模方案框架.md` | Step 1 建模方案出整题视角 + 跨题连续性 + 模型比选 |
| `bzd-cumcm-school-awards` | `references/学校画像与备赛评估.md` | 赛前 1 个月做学校画像 + 备赛分档评估 (备赛期) |

v1.5.9 加深 BZD `bzd-paper-aigc-auditor` v2.2 的借鉴 (Step 3-4 终审前):

| BZD 源 skill | li-mtrie 新增 | 何时用 |
|---|---|---|
| `bzd-paper-aigc-auditor` v2.2 | `references/llm-prompts/08-AIGC审计.md` + `references/审计报告模板.html` + `references/板块自查/09-AIGC审计.md` (v1.5.9 扩展) | Step 3-4 写完论文 + 终审前 1 小时, 跑两层审计 LLM agent 输出 HTML 报告 |

> **搬运策略**: v1.5.1 精简方法论 (不复制 BZD 完整 agents/scripts/assets, 详 CHANGELOG.md v1.5.8).
