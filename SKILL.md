...
**📌 周期起点信息边界原则 (v1.5.7.6 新增, 防数据泄露)** — **15 项必填之前必先做**:



数据泄露是历年 CUMCM 最大踩坑: 周期起点制定计划时**只能**基于已知信息 (历史数据 / 预报), **不能**用当天及以后实际数据 (评阅要点对应章节 "数据使用" 条款 (每年 4 月公布, 跑题前查最新), 包括隐含使用). 跑题前必须列出三类信息边界:
...
| **历史数据** (昨天/上周/历史平均) | ✓ 已知 | 附件 N 全年实际 row [day_idx-1] 等 | **周期起点预测输入** |

| **当天及以后实际** | ✗ 未知 | 附件 N row [day_idx] | **执行阶段 + 紧急购电对比基准** |

| **未来预报** (PV/负载预报) | ✓ 已知 (发布) | 附件 M 预报 | **周期起点预测 + 滚动调整输入** |



**判定规则**: 任何 周期起点 LP 求解, 预测输入必须是"历史数据" 或"未来预报", **绝对不能**是"当天实际". 紧急购电/调整偏差**只能在**"执行阶段 vs 周期起点计划" 的对比中产生, 不是 周期起点 LP 内部最小化目标.



**反模式 (踩过的坑, v1.5.7.6 实战案例)**:

1. ❌ 周期起点用附件 N 当天实际数据 (附件 2 row day_idx) 做 LP → 数据泄露 (隐含用未来信息)

2. ❌ 缩 g 上界 / 人造阴雨 / 改 LP 目标让紧急购电"非 0" → 物理造假, 不是题面要求

3. ❌ 用附件 M (Q3 预报) 做 Q2 → 越界, 题面规定 Q2 用 N+Q

4. ✅ 周期起点用前一天实际 / 历史平均 / 类别平均 (周末 vs 工作日) → 数据隔离正确, 紧急购电自然由"预测误差"触发
...
- Q3 紧急购电 329.7 万 kWh > Q2 259.0 万 kWh — **方向错, 说明滚动调整没起到压缩作用**, 附件 3 预报误差累积触发更多紧急购电. 正确做法可能是: ① 用**前一天实际** 替代附件 3 0:00 预报做 周期起点计划 (附件 3 误差更大), ② **减少滚动次数** (只在 12:00 一次调整, 不 6/12/18 三次)
...
**📌 预测方法选择 (v1.5.7.6 新增, 防 0:00 数据泄露)** — 题目说 "周期起点制定当天计划" 时:
...
**选择规则**: 题目要求"周期起点制定" + 有"已知数据" (附件 N 历史) → 选**前一天实际**或**同类型日平均**; 题目要求"滚动调整" + 有附件 M 预报 → 选**附件 M 预报 + 偏差分析**.
